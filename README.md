Badminton Player Ranking Prediction
...
Features Used
Prize Money: Total career prize money earned (scaled 0-1)
Win Rate: Wins/(Wins + Losses) from match history (0-1)
Points: BWF ranking points earned (scaled 0-1)
Target: Player rank (1 = best, higher numbers = worse ranking)
...
Key Findings
Points are most predictive: Largest weight magnitude (-134.86)
Prize money helps: Negative correlation as expected
Win rate counterintuitive: Positive weight suggests complexity in data
Model generalizes well: Reasonable predictions across rank ranges
...
Usage
Load and clean the BWF rankings data
Engineer features (win rate, points extraction)
Remove outliers and scale features
Train linear regression model with gradient descent
Evaluate predictions and analyze results
